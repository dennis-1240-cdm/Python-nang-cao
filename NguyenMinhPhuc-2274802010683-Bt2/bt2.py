...
def connect_to_db():
    try:
        ...
        connection = psycopg2.connect(
            ...
            password=password,
            port=port,
            options='-c client_encoding=UTF8'  # Mã hóa UTF-8  
        )
...
def create_login_page():
    ...
    entry_host.insert(0, "localhost")  # Host mặc định
    # Database
    ...
    entry_user.insert(0, "postgres")  # Username mặc định
    ...
    entry_port.insert(0, "5432")  # Port mặc định
...
def show_admin_page(connection):
    ...
    welcome_label = tk.Label(admin_frame, text=f"Quản lý người dùng", font=("Arial", 14))
    ...
    btn_add = tk.Button(button_frame, text="Add People", command=add_user)
    btn_add.grid(row=0, column=0, padx=5, pady=5)

    btn_update = tk.Button(button_frame, text="Update info", command=update_user)
    btn_update.grid(row=0, column=1, padx=5, pady=5)

    btn_delete = tk.Button(button_frame, text="Delete User", command=delete_user)
    btn_delete.grid(row=1, column=0, padx=5, pady=5)

    btn_reload = tk.Button(button_frame, text="Reload Table", command=lambda: reload_table('users'))
    btn_reload.grid(row=1, column=1, padx=5, pady=5)

    tables = get_tables(current_connection)

    tables_label = tk.Label(admin_frame, text="Danh sách bảng trong cơ sở dữ liệu:", font=("Arial", 12))
    tables_label.pack(pady=10)

    for table in tables:
        table_name = table[0]
        table_label = tk.Label(admin_frame, text=f"Table: {table_name}", font=("Arial", 10, "bold"))
    ...
    logout_button = tk.Button(admin_frame, text="Out", command=logout)
