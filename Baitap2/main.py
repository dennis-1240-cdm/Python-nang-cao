...
from cnndatabase import Database
...
class UserManagementApp:
    ...
    def create_connection_inputs(self):
        # Host
        tk.Label(self.login_frame, text="Host:").grid(row=0, column=0, padx=5, pady=5)
        ...
        # Database
        tk.Label(self.login_frame, text="Database:").grid(row=1, column=0, padx=5, pady=5)
        self.entry_database = tk.Entry(self.login_frame)
        self.entry_database.grid(row=1, column=1, padx=5, pady=5)

        # Username
        tk.Label(self.login_frame, text="Username:").grid(row=2, column=0, padx=5, pady=5)
        ...
        # Password
        tk.Label(self.login_frame, text="Password:").grid(row=3, column=0, padx=5, pady=5)
        self.entry_password = tk.Entry(self.login_frame, show="*")
        self.entry_password.grid(row=3, column=1, padx=5, pady=5)

        # Port
        tk.Label(self.login_frame, text="Port:").grid(row=4, column=0, padx=5, pady=5)
    ...
    def create_user_input_frame(self, parent):
        ...
        # Nhập tên người dùng
        tk.Label(input_frame, text="Tên người dùng:").grid(row=0, column=0, padx=5)
        self.entry_username = tk.Entry(input_frame)
        self.entry_username.grid(row=0, column=1, padx=5)

        # Nhập mật khẩu
        tk.Label(input_frame, text="Mật khẩu:").grid(row=1, column=0, padx=5)
        self.entry_password_new = tk.Entry(input_frame, show="*")
        self.entry_password_new.grid(row=1, column=1, padx=5)

        # Nhập vai trò
        tk.Label(input_frame, text="Vai trò:").grid(row=2, column=0, padx=5)
        self.entry_role = tk.Entry(input_frame)
        self.entry_role.grid(row=2, column=1, padx=5)

        # Nút Thêm người dùng
        add_button = tk.Button(input_frame, text="Thêm người dùng", command=self.add_user)
        add_button.grid(row=3, column=0, columnspan=2, pady=5)

        # Nút Xóa người dùng
        delete_button = tk.Button(input_frame, text="Xóa người dùng", command=self.delete_user)
        delete_button.grid(row=4, column=0, columnspan=2, pady=5)

        # Nút Cập nhật người dùng
        update_button = tk.Button(input_frame, text="Cập nhật người dùng", command=self.update_user)
    ...
    def update_user(self):
        ...
        if selected_item:
            ...
            self.user_management.update_user(user_id, new_username, new_password, new_role, self.list_widget)  # Gọi hàm cập nhật người dùng
        else:
            messagebox.showwarning("Cảnh báo", "Vui lòng chọn người dùng để cập nhật.")  # Thông báo cảnh báo nếu không chọn

if __name__ == "__main__":
    win = tk.Tk()
    app = UserManagementApp(win)  # Khởi tạo ứng dụng
    win.mainloop()  # Bắt đầu vòng lặp chính
