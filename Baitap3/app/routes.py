...
def index():
    ...
    metadata = MetaData()
    categories_table = Table('danhmuc', metadata, autoload_with=engine)
    products_table = Table('tainghe', metadata, autoload_with=engine)
...
def get_cart():
    ...
    engine = get_engine()
    metadata = MetaData()
    carts_table = Table('giohang', metadata, autoload_with=engine)
    products_table = Table('tainghe', metadata, autoload_with=engine)
    ...
    try:
        with engine.connect() as connection:
            query = select(
                carts_table.c.id,
                carts_table.c.id_product,
                carts_table.c.number_product,
                products_table.c.nameproduct,
                products_table.c.price,
                products_table.c.image
            ).where(
                (carts_table.c.id_user == user_id) &
                (carts_table.c.id_product == products_table.c.id)
            )
            result = connection.execute(query)
            cart_items = [
                {
                    ...
                    "prod_qty": row.number_product,
                    "prod_name": row.nameproduct,
                    "prod_price": row.price,
                    "prod_img": url_for('static', filename=f"uploads/{row.image}")
                } for row in result
            ]
    ...
    try:
        with engine.connect() as connection:
            ...
            query = select(carts_table).where(
                (carts_table.c.id_user == user_id) &
                (carts_table.c.id_product == prod_id)
            )
            ...
            if existing_cart_item:
                ...
                update_query = carts_table.update().where(
                    carts_table.c.id == existing_cart_item.cart_id
                ).values(number_product=carts_table.c.number_product + prod_qty) 
    ...
    try:
        with engine.connect() as connection:
            ...
            if existing_cart_item:
                ...
                update_query = carts_table.update().where(
                    carts_table.c.id == existing_cart_item.cart_id
                ).values(number_product=prod_qty)  # Cập nhật số lượng mới
                connection.execute(update_query)
...
def search():
    ...
    query = products_table.select().where(products_table.c.nameproduct.ilike(f'%{search_query}%'))
...
def show_category(id, category_name):
    ...
    with engine.connect() as connection:
        ...
        query = select(products_table).where(
            products_table.c.categoryid == id,
        ).limit(per_page).offset(offset)
        result = connection.execute(query).mappings()
        products = [dict(row) for row in result]

        # Tính tổng số trang
        total_products_query = select(func.count(products_table.c.id)).where(products_table.c.categoryid == id)
        total_products_result = connection.execute(total_products_query).scalar()
        total_pages = ceil(total_products_result / per_page)
...
def admin_board():
    ...
    engine = create_engine(f'postgresql+psycopg2://sach:123@localhost:5432')
    try:
        with engine.connect() as connection:
            databases = ["tainghe"]
    except SQLAlchemyError as e:
    ...
    try:
        ...
        with engine.connect() as connection:
            update_query = products_table.update().where(products_table.c.id == product_id).values(
                ...
                status=status,
                categoryid=category_id,
                image=image_filename if image_filename else products_table.c.image
            )
            connection.execute(update_query)
            connection.commit()
...
def delete_product():
    try:
        ...
        with engine.connect() as connection:
            ...
            query = select(products_table.c.image).where(products_table.c.id == product_id)
            result = connection.execute(query).fetchone()
